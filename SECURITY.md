...
- Rendered vault values are inserted so common config file formats remain syntactically valid. Per-reference `format="..."` overrides and the `--raw` flag disable that protection — you are responsible for quoting in the template when using them.
